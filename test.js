...
// Function to create a widget from the message
...
function replaceAdsWithWidget() {
  ...
  adElements.forEach(ad => {
    ...
    console.log('Ad removed:', ad); // Log each removed ad
  });

  createWidget(); // Create widget after ad removal
}

// Function to create and display the widget
function createWidget() {
  if (widgetCreated) {
    console.log('Widget already created.');
    return; // Avoid duplicate widgets
  }

  widgetCreated = true;
  const widgetMessage = widgetMessages[Math.floor(Math.random() * widgetMessages.length)];
  createWidgetFromMessage(widgetMessage);
}

setInterval(replaceAdsWithWidget, 3000); // Run every 3 seconds to check for and replace ads
